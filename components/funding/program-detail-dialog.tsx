...
export function ProgramDetailDialog({ program }: { program: FundingProgram }) {
  const checklist = useAppStore((s) => s.checklist);

  // Cross-module link: most programs need a Canadian entity, which is the
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        ...
        <section aria-labelledby={`eligibility-${program.id}`} className="mt-6">
          <h3
            ...
            <div className="text-sm leading-relaxed">
              {registrationTracked ? (
                <p className="text-teal-900">
                  This programme requires a Canadian-registered entity, and your provincial
                  registration is marked complete on your compliance checklist.
                </p>
              ) : (
                <>
                  <p className="text-amber-900">
                    This programme requires a Canadian-registered entity. Complete your
                    extra-provincial registration before applying, or your application will be
                    returned.
                  </p>
                  <Button asChild variant="link" className="mt-1 h-auto p-0 text-amber-900">
                    <Link href="/dashboard/guide">Open the Province Guide</Link>
                  </Button>
                </>
              )}
            </div>
        ...
        <p className="mt-6 flex gap-2 text-xs leading-relaxed text-slate-500">
          ...
          Programme details are illustrative planning estimates for this demonstration. Confirm
          current criteria, amounts, and intake dates with the administering body before applying.
        </p>
      </DialogContent>
    </Dialog>
  );
